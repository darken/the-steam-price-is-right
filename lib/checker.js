...
checker.handleResponseData = function (data) {
  ...
  if (soundAlarm) sayBuy(this.itemName, number);
};

function sayBuy(itemName, currentPrice) {
  var phrase = util.format('Buy %s for %d dollars', itemName, currentPrice);
  notifier(phrase, function (error) { if (error) throw error; });
}
...
function createChecker() {
  ...
  checker.init.apply(newChecker, arguments);
  return newChecker;
}
